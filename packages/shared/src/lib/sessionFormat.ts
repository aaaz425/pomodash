import type { TimerMode, FocusPeriod } from '../types/timer';
...
export function groupSessionsByDate<T extends { startedAt: string; focusSeconds: number }>(
  sessions: T[],
  ref?: Date,
): SessionGroup<T>[] {
...
export function getSessionsForDate<T extends { startedAt: string }>(
  sessions: T[],
  date: Date,
): T[] {
