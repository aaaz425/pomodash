...
- [x] `feat/share-card` — 집중 요약 공유 카드 (클라이언트 Canvas 생성)

## Phase 7 — 백엔드 & 동기화

- [x] `chore/cleanup-legacy-copy` — 로그인 없음 포지셔닝 문구 및 PostHog 트래킹 제거
- [x] `chore/supabase-migration` — localStorage → Supabase 마이그레이션 (로그인, 동기화)
- [x] `style/skeleton-ui` — 페이지별 스켈레톤 로딩 UI (supabase-migration 완료 후)
...
- [>] `feat/rn-journal` — 기록(세션 히스토리 리스트/캘린더/상세편집)
- [ ] `feat/rn-dashboard` — 대시보드(집중 시간 집계·차트·스트릭)
