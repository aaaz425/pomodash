...
포트폴리오 프로젝트로서 작업 단위와 사고 과정이 GitHub에 명확히 보이도록 설계한다.
...
모든 작업은 기능 브랜치에서 진행하고, PR을 통해서만 main에 반영한다.
...
PR은 포트폴리오에서 작업 단위와 사고 과정을 보여주는 핵심 도구다.
브랜치 단위 = PR 단위. `.github/pull_request_template.md` 형식을 따른다.

### PR 단위 기준

브랜치 단위와 동일하게 **단일 목적**을 유지한다.
PR description의 "왜 이렇게 구현했나" 항목이 포트폴리오에서 가장 중요한 부분이다 — 구현 결정 이유를 구체적으로 남긴다.
...
### Squash merge 후 브랜치 작업 시 필수

Squash merge는 브랜치 커밋을 새 해시로 교체해 main에 올린다.
이후 같은 브랜치에서 계속 작업하면 이미 머지된 커밋이 다음 PR에 다시 나타난다.

**새 브랜치를 만들거나, 이전 PR이 머지된 브랜치에서 계속 작업할 때 항상 먼저 실행:**
...
브랜치 내 작업 커밋은 자유롭게 써도 된다 — squash merge로 main에서 사라진다.
**main에 남는 squash commit 메시지**만 아래 형식을 따른다.
