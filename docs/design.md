...
description: "딥 네이비 캔버스 위에 민트 단일 액센트. 절제된 다크 테마로 집중 타이머 앱의 목적인 '집중'을 시각적으로 구현한다. Linear의 디자인 철학(단일 액센트 / 표면 계층 / 화면이 주인공)을 포모도로 앱에 맞게 재해석."
source: "linear.app DESIGN.md (awesome-design-md) 기반 번역 + Pomodash 커스터마이징"
---
...
색상은 딥 네이비 + 민트 단 두 축으로만 말한다. 장식은 없다.

**핵심 원칙:**
- 다크 모드 전용 (라이트 모드 미제공)
- 단일 크로매틱 액센트 (민트 `#10d9a0`) — 타이머 링, CTA 버튼, 포커스 링에만 사용
- 표면 계층으로 깊이 표현 — 그림자 없음, 테두리 없이 배경색 차이로만
...
| 역할 | 폰트 | fallback |
|------|------|---------|
| 기본 (본문, UI) | **Inter** | `-apple-system, system-ui, Segoe UI` |
| 타이머 숫자 | **Geist Mono** | `ui-monospace, SF Mono, Menlo` |
| 코드 / 로그 | **Geist Mono** | `ui-monospace` |

Next.js `next/font`로 Inter + Geist Mono를 로드한다.
...
> 텍스트 와이어프레임. 실제 구현 시 이 구조를 기준으로 컴포넌트를 배치한다.

### 7-1. 타이머 메인 화면 (`/`)

```
┌──────────────────────────────────────────────┐
│ [Pomodash 로고]           [설정⚙] [대시보드📊] │  ← top-nav (56px, canvas 배경)
├──────────────────────────────────────────────┤
│                                              │
│              ┌─────────────┐                 │
│              │  집중 중 •  │  ← SessionBadge  │
...
│           └──────────────────┘               │
│                                              │
│    "조급해하지 않아도 괜찮아. 지금 충분해."     │  ← 동기부여 메시지
│                                              │
│                        [✕ 집중 모드 종료]     │
└──────────────────────────────────────────────┘
```

### 7-2. 설정 패널 (우측 슬라이드 시트 또는 모달)
...
## 9. Tailwind CSS v4 토큰 매핑 (참조용)

> `app/globals.css`의 shadcn CSS 변수와 대응. 구현 시 이 매핑을 기준으로 한다.

```css
:root {
  /* 배경 */
  --background: 220 30% 5%;        /* canvas #07090f */
  --card: 220 25% 8%;              /* surface-1 #0d1117 */
  --popover: 220 22% 10%;          /* surface-2 #111827 */
  --muted: 220 20% 15%;            /* surface-3 #1a2332 */

  /* 텍스트 */
  --foreground: 210 40% 96%;       /* ink #f0f4f8 */
  --muted-foreground: 215 16% 57%; /* ink-muted #94a3b8 */
  --card-foreground: 210 40% 96%;

  /* 액센트 (민트) */
  --primary: 162 88% 46%;          /* #10d9a0 */
  --primary-foreground: 220 30% 5%; /* 다크 텍스트 on primary */
  --ring: 162 88% 46%;             /* 포커스 링 */

  /* 테두리 */
  --border: 215 28% 17%;           /* hairline #1e2d3d */
  --input: 215 28% 17%;

  /* 시맨틱 */
  --destructive: 0 72% 51%;        /* error #ef4444 */
}
```
