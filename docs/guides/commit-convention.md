...
**본문은 모든 섹션을 개조식(bullet)으로 짧게 쓴다 — 문단형 프로즈 금지.** 해당 없는 섹션(스크린샷 등)과 이번 PR에서 실제로 확인하지 않은 체크리스트 항목은 채우지 말고 통째로 삭제한다. 템플릿 기본 체크리스트를 기계적으로 다 채우지 않는다.

### PR 단위 기준

브랜치 단위와 동일하게 **단일 목적**을 유지한다.
PR description에서 "왜 이렇게 구현했나" 항목이 가장 중요하다 — 구현 결정 이유를 짧은 불릿으로 남긴다.
