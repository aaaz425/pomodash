...
export default function DashboardPage() {
  ...
  return (
    <main className="flex flex-col gap-6 p-8 md:p-10 max-w-5xl">
      {/* Header */}
      ...
      <div className="flex gap-3">
        ...
        <StatCard
          ...
          sub={sessionCountSub}
        />
        <StatCard label="연속 집중일" Icon={Flame} value={`${streakDays}일`} sub="현재 연속 기록" />
        <StatCard
          label="세션 평균"
          Icon={ChartColumn}
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="flex flex-col gap-3 p-5 rounded-lg border border-border bg-card">
          <p className="text-sm font-semibold text-foreground">최근 세션</p>
          <RecentSessions sessions={recentSessions} tasks={tasks} categories={categories} />
        </div>
      </div>
    </main>
  );
